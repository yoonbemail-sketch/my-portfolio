...
Hall calls are assigned with a nearest-car cost **when the passenger appears**, then **stick** — a later idle car that is closer does not steal the call. SCAN delays opposite-direction pickups until the assigned car finishes its current direction.

**Example** (Copy debug, seed 42, Stay, evening, tick 650, IdleFrac 62%):
...
Cost *now* prefers E3/E4, but `#76` was assigned at tick 619 and never reassigned. Parking (where idle cars wait) and **call reassignment** are separate levers — high IdleFrac with long waits often means free cars exist, but sticky dispatch will not hand them the call.
...
Research directions (future code): **hall-call reassignment**, arrival-probability parking, MDP lobby-car counts for up-peak, proactive standby scores, and **service zoning** when saturated.
