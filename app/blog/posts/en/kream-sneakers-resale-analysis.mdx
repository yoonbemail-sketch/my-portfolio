...
- **Monitor:** high volume + elevated premium with a clear size split (e.g. Jordan concentration and size heatmap)
- **Avoid / wait:** thin trade history or premium that only looks good on a short window (Signals + period what-if)
- **Revisit:** weekend vs weekday premium gaps or delivery-type shifts that change expected net

Sample scale behind those views: ~**3M** trades, ~**912bn** KRW sales, ~**59%** average premium — useful for relative ranking, not a guarantee on the next trade.
