...
Environment
  ├─ 아파트 OD (lobby↔home)
  ├─ 트래픽 설정 (peak / arrival / interfloor)
  └─ 고정 시나리오 시드 (공정한 재현)
        │
        ▼
Measurement
...
파킹은 차가 유휴일 때만 효과가 있습니다. arrival을 올려 뱅크를 바쁘게 유지하면 전략 격차가 줄고 IdleFrac가 떨어집니다 → “항상 붐비면 파킹이 아니라 존.”
...
데모는 아직 **파킹만** 분리합니다. Zoning은 문서상 다음 레버이지 코드에는 없습니다.

## 2. IdleFrac

`IdleFrac = (IDLE|PARKING car-ticks) / (ticks × elevators)` — 진단용이며 Rank-by 목표가 아닙니다.
...
**서비스 층 (출시됨, Rank-by 목표 아님):** SCAN은 한 방향을 끝낸 뒤 반전합니다. 도어가 열리면 그 층의 **같은 방향** 대기자는 모두 탈 수 있습니다. 파킹·배정 실험 전에 collective 동작이 말이 되게 합니다.

**Sticky:** 도착 순간 한 번 배정. **Reassign:** 매 틱 대기 홀 호출 배정을 지우고 다시 스코어.
...
더 가까운 IDLE이 sticky에서는 호출을 못 뺏습니다. SCAN 때문에 반대 방향 탑승은 더 늦어집니다.
...
베이스라인: Stay / Lobby / Mid / Spread / Demand. Batch N + Rank-by는 “*이* 레짐에서 누가 이기는가?”에 답합니다 — 전역 최적 아님.

Policy / Environment / Playback 패널은 나중에 zoning을 넣을 자리를 남겨 둡니다.

---

## Limitations (한계)

이것은 **합성 의사결정 실험**이지, 캘리브레이션된 엘리베이터 컨트롤러 연구가 아닙니다.

- 도착은 Bernoulli(틱당 최대 1명)이며 실제 빌딩 로그에 맞춘 것이 아닙니다.
- 시간은 이산 tick; dwell/capacity는 단순 설정입니다.
- 홀 비용은 거리·부하·가벼운 방향항 — **그룹 ETA / destination control 아님**.
- 실제 뱅크의 OD·도어타임·에너지 없음 → 순위를 **건물 정책 추천으로 쓰지 말 것**.
- Batch **winRate**는 동점을 각 전략에 세어 합이 100%를 넘을 수 있습니다.

연습 대상은 **레버 분리, 공정 A/B, 레짐 진단**입니다. 실무라면 arrival/OD/dwell을 먼저 맞춘 뒤 같은 Batch 프레임을 재사용합니다.
...
1. **왜 avg wait / max wait / empty travel를 나누나?**  
   단일 최적 없음 — evening 대기는 Lobby, 공차는 Stay, morning은 Spread. Rank-by로 레짐별 tradeoff를 드러냅니다.

2. **reassign에서 Mid 평균 대기가 나빠진 이유?**  
   근시안적인 유휴 가로채기: 빈 중간층 차가 상행 차가 en-route로 처리할 호출을 가로챕니다. Evening N=100: Mid 4.17 → **5.02** ([COMPARE](benchmarks/COMPARE.md)). Stay max는 방치된 호출 해소로 개선됩니다.

3. **IdleFrac가 아주 낮을 때 파킹을 더 튜닝할까?**  
   No. IdleFrac **5%** high-arrival Batch에서 Stay−best 격차는 **0.90** — 파킹은 거의 안 움직입니다. 다음 레버는 **zoning**(문서만, 미구현).
...
5. **실무에서 이 시뮬을 어떻게 쓰나?**  
   “어떤 트래픽에서 어떤 idle-placement?”를 실험하는 하네스입니다. 캘리브레이션·현장 지표 없이 벤더 컨트롤러를 바꾸라는 증거가 아닙니다.
